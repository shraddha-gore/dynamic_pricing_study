...
Select candidate with maximum predicted_revenue

If multiple candidates share the same revenue:

choose candidate with smallest distance to base_price

if still tied, choose the lower candidate_price
...
Identify the ML-optimal candidate price:

ml_price = ML-optimal candidate_price
...
Reflects common retail operational limits

Maintains continuity in pricing trajectories
...
smoothed_price =
    HYBRID_SMOOTHING_ALPHA * clamped_price
    + (1 - HYBRID_SMOOTHING_ALPHA) * previous_price
Rationale

Dampens short-term volatility

Ensures gradual price adjustments

Mimics real-world pricing moderation practices

Step 4 - Candidate Grid Compliance

Strategies must select from the simulator’s candidate grid.

The final price is selected as the candidate closest to the smoothed value:

chosen_price =
    candidate with minimum distance to smoothed_price

Tie-breaking rules:

closest to smoothed_price

closest to base_price

lower candidate_price
...
previous_price_by_product[stock_code]
...
for the first day of each product.

Strategy Characteristics

Hybrid pricing aims to achieve:

revenue performance close to ML pricing

significantly reduced price volatility

operationally realistic pricing trajectories
