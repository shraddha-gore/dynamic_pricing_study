...
## TODO - Refactors and Improvements

### Refactor Priorities
- Revisit the split between `config.py` and `pipeline/execution.py` and decide whether execution-domain constants should be partially centralized in `config.py`
- Reduce duplication between CLI command wiring in `main.py`, runtime registry definitions, and logging target declarations
- Review step/module naming consistency across the codebase (`build`, `model training`, `simulation`, `evaluation`, `validation`) and standardize any remaining mixed phrasing

### Quality and Testing
- Add a project lint command and shared lint configuration so style and import issues can be checked consistently before validation runs
- Add automated tests for CLI argument handling and command dispatch in `main.py`
- Add contract tests for frozen schemas and critical artifact loaders/validators in `utils/data_contracts.py`
- Add strategy-level regression tests to verify deterministic candidate selection and chosen-price outputs for `rule`, `ml`, and `hybrid`
- Add an automated smoke test that exercises `--build`, `--simulate rule`, `--evaluate`, and `--validate` on the frozen sample workflow

### Documentation and Usability
- Add a concise `README.md` run guide so basic project execution does not depend on reading the full implementation roadmap
- Add a short artifact map documenting which commands produce which files under `data/`, `results/`, `models/`, and `logs/`
- Document expected runtime duration and compute cost for each command to make reruns easier to plan
- Perform a dashboard review pass to verify what should be shown by default, what should be removed, which comparisons need clearer labeling, and what customization controls or layout changes would improve usability

### Analytical Improvements
- Review demand model quality and decide whether the current Linear Regression baseline should remain the only frozen model or be compared with a stronger forecasting baseline
- Expand validation scenarios beyond the current two hybrid-parameter perturbations to cover broader robustness checks
- Consider adding experiment tracking metadata for major runs so frozen outputs can be linked to command, date, and key parameter settings more explicitly
